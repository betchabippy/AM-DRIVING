...
import { Plus, Check, Shield, Camera, X, Pencil } from 'lucide-react'
...
function CarModal({
  title, onClose, onSave, saving,
  make, setMake, model, setModel, year, setYear,
  color, setColor, spec, setSpec, nickname, setNickname,
  club, setClub, photoPreview, setPhotoFile, setPhotoPreview, fileInputRef
}: any) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
      <div className="w-full max-w-md card p-6 animate-slide-up max-h-[90vh] overflow-y-auto">
          ...
          {photoPreview ? (
            <div className="relative h-40 rounded-xl overflow-hidden">
              ...
              <button onClick={() => fileInputRef.current?.click()}
                className="absolute bottom-2 right-2 flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-black/60 text-white hover:bg-black/80">
        ...
        <button onClick={onSave} disabled={saving || !make || !model || !year}
...
export default function GaragePage() {
  ...
  const openEdit = (car: any) => {
    setEditingCar(car)
    setMake(car.make || 'Aston Martin')
    setModel(car.model || '')
    setYear(car.year?.toString() || '')
    setColor(car.color || '')
    setSpec(car.spec || '')
    setNickname(car.nickname || '')
    setClub(car.club_name || 'None')
  ...
  const uploadPhoto = async (carId: string): Promise<string | null> => {
    ...
    const { error } = await supabase.storage.from('car-images').upload(path, photoFile, { upsert: true })
  ...
  const handleAddCar = async () => {
    ...
    if (photoFile) photoUrl = await uploadPhoto(carId)

    const { data, error } = await supabase.from('cars').insert({
      id: carId, user_id: user.id, make, model,
      year: parseInt(year), color: color || null, color_hex: colorHex,
      spec: spec || null, nickname: nickname || null,
      club_name: club === 'None' ? null : club,
      club_badge: club === 'None' ? 'none' : 'self-declared',
      is_primary: isPrimary, photo_url: photoUrl,
    }).select().single()

    setSaving(false)
    if (error) { alert(JSON.stringify(error)); return }
    if (data) { setCars(prev => [...prev, data]); setShowAddCar(false); resetForm() }
  }

  const handleEditCar = async () => {
    if (!user || !editingCar || !make || !model || !year) return
    setSaving(true)
    const colorHex = COLOR_MAP[color] || editingCar.color_hex || '#1a1a1a'
    let photoUrl = editingCar.photo_url
    if (photoFile) photoUrl = await uploadPhoto(editingCar.id)
    if (!photoPreview && !photoFile) photoUrl = null

    const { data, error } = await supabase.from('cars').update({
      make, model, year: parseInt(year),
      color: color || null, color_hex: colorHex,
      spec: spec || null, nickname: nickname || null,
      club_name: club === 'None' ? null : club,
      club_badge: club === 'None' ? 'none' : 'self-declared',
      photo_url: photoUrl,
    }).eq('id', editingCar.id).select().single()

    setSaving(false)
    if (error) { alert(JSON.stringify(error)); return }
    if (data) {
      setCars(prev => prev.map(c => c.id === data.id ? data : c))
      setEditingCar(null); resetForm()
    }
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 pb-24 md:pb-8">
      ...
      <div className="grid grid-cols-3 gap-3 mb-10">
        {[{ label: 'Cars', value: cars.length }, { label: 'Drives', value: 0 }, { label: 'Miles', value: 0 }].map(({ label, value }) => (
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { resetForm(); setShowAddCar(true) }}
          className="flex items-center gap-1.5 text-xs text-gold-400 hover:text-gold-200 transition-colors">
            ...
            <div key={car.id} className="card overflow-hidden">
              <div className="h-48 relative overflow-hidden" style={{ background: `${car.color_hex || '#1a1a1a'}30` }}>
                ...
                <button onClick={() => openEdit(car)}
                  className="absolute top-3 right-3 flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-black/60 text-white hover:bg-black/80 transition-colors">
                  <Pencil size={11} /> Edit
                </button>
              </div>
              <div className="p-5">
      ...
      {showAddCar && (
        <CarModal
          title="Add a car"
          onClose={() => { setShowAddCar(false); resetForm() }}
          onSave={handleAddCar}
          saving={saving}
      ...
      {editingCar && (
        <CarModal
          title="Edit car"
          onClose={() => { setEditingCar(null); resetForm() }}
